...
int pds_open(char* repo_name, int rec_size)
{
    ...
    FILE *file_ptr = fopen(data_file_name,"rb+");
    ...
    while(1 == 1) // always true
    {
        ...
        else if(!feof(repo_handle.pds_ndx_fp))
        {
            ...
            *temp_offset = temp.offset;
            bst_add_node(&repo_handle.pds_bst,temp.key,temp_offset);
        }
        else
        {
            return PDS_SUCCESS;
...
int put_rec_by_key(int key, void*rec)
{
    ...
    if(repo_handle.repo_status == PDS_REPO_OPEN)
    {
        ...
        int *position = (int*) malloc(100 * sizeof(int));

        *position = (int) ftell(repo_handle.pds_data_fp);
        ...
        {
            bst_add_node(&repo_handle.pds_bst,key,position);
            return PDS_SUCCESS;
        }
    }
    else
